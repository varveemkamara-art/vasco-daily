...
function Settings() {
  ...
  async function handleDeleteAccount() {
    ...
    // Deletes profile row; cascading foreign keys remove tasks, categories, etc.
    // Note: this does NOT delete the actual auth.users login record —
    // that requires an admin-level operation we'll cover separately if needed.
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-slate-800 p-4 rounded-lg">
        ...
        <p className="text-xs text-slate-400 mb-2">Email: {user.email}</p>
      ...
      <div className="bg-slate-800 p-4 rounded-lg">
        ...
        <div className="space-y-3">
          ...
          <div className="pt-2 border-t border-slate-700">
            <span className="text-slate-300 block mb-2">Email Reminders</span>
            <div className="flex gap-2">
              <input
                type="email"
                ...
                className="flex-1 p-2 rounded bg-slate-700 text-white outline-none text-sm"
              />
              <button
      ...
      {/* Preferences */}
      <div className="bg-slate-800 p-4 rounded-lg">
        <h2 className="text-white font-semibold mb-3">Preferences</h2>

        <div className="space-y-3">
          <div>
            <label className="text-sm text-slate-300 block mb-1">Timezone</label>
            <select
              value={settings.timezone}
              onChange={(e) => updateSettings({ timezone: e.target.value })}
              className="w-full p-2 rounded bg-slate-700 text-white outline-none text-sm"
            >
              {TIMEZONES.map((tz) => (
          ...
          <div>
            <label className="text-sm text-slate-300 block mb-1">Clock Format</label>
            <div className="flex gap-2">
              <button
      ...
      {/* Data & Account */}
      <div className="bg-slate-800 p-4 rounded-lg">
        <h2 className="text-white font-semibold mb-3">Data & Account</h2>

        <button
          onClick={handleExportData}
          className="w-full bg-slate-700 hover:bg-slate-600 text-white p-2 rounded text-sm mb-3"
        >
          Export My Data (JSON)
        </button>
        ...
        <div className="pt-3 border-t border-slate-700">
          ...
          <input
            ...
            className="w-full p-2 rounded bg-slate-700 text-white outline-none text-sm mb-2"
          />
          <button
