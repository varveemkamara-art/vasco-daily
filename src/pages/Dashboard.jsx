function Dashboard({ occurrences, userEmail, fullName }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-slate-800 p-4 rounded-lg">
        <p className="text-sm text-slate-300 mb-2">Today's Progress</p>
      ...
      <div className="bg-slate-800 p-4 rounded-lg">
        <h3 className="text-sm font-semibold text-white mb-2">Upcoming</h3>
        {upcoming.length === 0 ? (
          <p className="text-sm text-slate-400">Nothing upcoming.</p>
        ) : (
          <ul className="space-y-1">
            {upcoming.map((o) => (
              <li key={o.occurrenceId} className="text-sm text-slate-300">
